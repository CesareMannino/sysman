...
function checkForNewRecords() {
  ...
  db.query(query, async (err, results) => {
    if (err) {
      ...
      if (results.length > 0) {
        lastCheckedUserId = Math.max(...results.map((user) => user.user_id));
      }
    }
  });
}
